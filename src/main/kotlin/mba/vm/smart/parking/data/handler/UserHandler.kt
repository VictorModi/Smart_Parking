package mba.vm.smart.parking.data.handler

import mba.vm.smart.parking.UserType
import mba.vm.smart.parking.data.DataResult

/**
 * SmartParking - mba.vm.smart.parking.data.handler
 * @description TODO: coming soon.
 * @author VictorModi
 * @email victormodi@outlook.com
 * @date 2024/6/29 上午10:48
 */
//object UserHandler : BaseDataHandler() {
//    /**
//     * 读取权限的用户类型数组，需由子类实现。
//     */
//    override val readPermission: Array<UserType> = arrayOf(UserType.SUPER_ADMIN, UserType.SYSTEM_ADMIN)
//
//    /**
//     * 写入权限的用户类型数组，需由子类实现。
//     */
//    override val writePermission: Array<UserType> = arrayOf(UserType.SUPER_ADMIN, UserType.SYSTEM_ADMIN)
//
//    /**
//     * 抽象方法，需由子类实现，定义选择操作。
//     * @param data 请求的数据。
//     * @return 数据操作的结果。
//     */
//    override fun select(data: Map<String, Any>?): DataResult {
//        TODO("Not yet implemented")
//    }
//
//    /**
//     * 抽象方法，需由子类实现，定义插入操作。
//     * @param data 请求的数据。
//     * @return 数据操作的结果。
//     */
//    override fun insert(data: Map<String, Any>?): DataResult {
//        TODO("Not yet implemented")
//    }
//
//    /**
//     * 抽象方法，需由子类实现，定义更新操作。
//     * @param data 请求的数据。
//     * @return 数据操作的结果。
//     */
//    override fun update(data: Map<String, Any>?): DataResult {
//        TODO("Not yet implemented")
//    }
//
//    /**
//     * 抽象方法，需由子类实现，定义删除操作。
//     * @param data 请求的数据。
//     * @return 数据操作的结果。
//     */
//    override fun delete(data: Map<String, Any>?): DataResult {
//        TODO("Not yet implemented")
//    }
//}
